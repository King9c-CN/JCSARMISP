...
set rc [catch {
  open_project {m0soc.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {../syn_1/m0soc_gate.db}
  read_sdc ../../m0soc.sdc
  read_sdc -ip fifo_data ../../../rtl/al_ip/fifo_data.tcl
  read_sdc -ip fifo_data_out ../../al_ip/fifo_data_out.tcl
  read_sdc -ip sd2isp_fifo ../../al_ip/sd2isp_fifo.tcl
  place
  export_db {m0soc_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file m0soc_phy.area
  export_db {m0soc_pr.db}
  start_timer
  report_timing -mode FINAL -net_info  -rpt_autogen true -file m0soc_phy.timing
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid m0soc_inst.bid
  bitgen -bit "m0soc.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
